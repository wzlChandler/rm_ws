...
def main():
    ...
    # 手掌位置(碗中心)
    (h, _) = listener.lookupTransform("world", "hand_base_link", rospy.Time(0))
    rospy.loginfo("手掌位置: %s", [round(v, 3) for v in h])

    # 苹果放到手掌正下方(桌面高度 0.155)
    set_apple(set_srv, h[0], h[1], 0.155)
    rospy.sleep(2.0)
    ...
    # 上收提起
    arm_move(arm_pub, [0.0, 0.7, -0.5, 0.0, 0.0, 0.3, 0.0], 5.0)
    rospy.sleep(6.0)
    a2 = get_apple(get)
