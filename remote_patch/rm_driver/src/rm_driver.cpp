...
void Read_Modbus_Holding_Registers_Callback(const rm_msgs::Read_TCPandRTU msg)
{
    ...
    {
        ROS_INFO("Read tool Modbus holding registers with SDK-compatible command!\n");
        int res = 0;
        if (msg.num == 1)
        {
            modbus_data.read_holding_registers.data.resize(1);
            res = Read_Holding_Registers_Cmd(
                static_cast<uint8_t>(msg.port), msg.address, msg.device);
        }
        else
        {
            res = Read_Multiple_Holding_Registers_Cmd(
                static_cast<uint8_t>(msg.port), msg.address, msg.num, msg.device);
        }
        if(res == 0){
            // ROS_INFO("Read tool Modbus holding registers callback success!\n");
        }
        else{
            ROS_ERROR("Read tool Modbus holding registers callback cmd failed!\n");
        }
    }
    else
    {
...
void Write_Modbus_Registers_Callback(const rm_msgs::Write_TCPandRTU msg)
{
    ...
    {
        ROS_INFO("Write fourth-generation RTU registers callback!\n");
        if (msg.data.empty() || (msg.data.size() % 2) != 0)
        {
            ROS_ERROR("Invalid register byte count: %zu", msg.data.size());
            return;
        }

        std::vector<uint16_t> data(msg.data.begin(), msg.data.end());
        int res = 0;
        res = Write_Registers_Cmd(
            static_cast<uint8_t>(msg.port), msg.address,
            static_cast<uint32_t>(msg.data.size() / 2), data, msg.device);
        if(res == 0){
            // ROS_INFO("Write fourth-generation RTU registers callback success!\n");
        }
        else{
            ROS_ERROR("Write fourth-generation RTU registers callback cmd failed!\n");
        }
    }
    else
    {
...
// 声明抓取指令的订阅器
ros::Subscriber sub_visual_grasp;

// 我们的视觉抓取回调函数
void Visual_Grasp_Callback(const std_msgs::Int32::ConstPtr& msg)
{
    int value = msg->data;
    
    // 原汁原味复刻你的 Python 数组逻辑: 12个元素的数组
    std::vector<uint16_t> data(12, 0); 
    
    if (value == 0) {
        ROS_INFO("✋ [视觉抓取] 执行张开手指...");
    } else {
        ROS_INFO("✊ [视觉抓取] 执行闭合手指 (力度: %d)...", value);
        // 远端稍微小一点(80%)
        for (int i = 0; i < 10; ++i) {
            if (i % 2 == 0) data[i] = value;
            else data[i] = static_cast<int>(value * 0.8);
        }
    }
    
    // 【最核心的魔法】：直接调用底层的 C++ SDK，绕过有Bug的ROS话题！
    // 对应你的Python代码: port=1, address=1135, num=6, data, device=2
    int res = Write_Registers_Cmd(1, 1135, 6, data, 2);
    
    if (res == 0) {
        ROS_INFO("✅ 灵巧手驱动成功!");
    } else {
        ROS_WARN("❌ 灵巧手驱动失败, 错误码: %d", res);
    }
}
...
char udp_socket_buffer[8192];

bool read_data()
{
    memset(udp_socket_buffer, 0, sizeof(udp_socket_buffer));

    ssize_t numBytes = recvfrom(Udp_Sockfd, udp_socket_buffer, sizeof(udp_socket_buffer) - 1, 0,
    ...
    if (numBytes >= 2 && udp_socket_buffer[numBytes - 2] == 0X0D && udp_socket_buffer[numBytes - 1] == 0X0A)
    {
        return true;
    }
    else
    {
        const unsigned int penultimate = numBytes >= 2 ? static_cast<unsigned char>(udp_socket_buffer[numBytes - 2]) : 0;
        const unsigned int last = numBytes >= 1 ? static_cast<unsigned char>(udp_socket_buffer[numBytes - 1]) : 0;
        ROS_WARN_THROTTLE(5.0, "UDP packet discarded: bytes=%zd trailing=0x%02X 0x%02X", numBytes, penultimate, last);
        return false;
    }
}
...
void udp_timer_callback(const ros::TimerEvent)
{
    if(read_data()==true)
    {
        const int parse_result = Parser_Udp_Msg(udp_socket_buffer);
        if (parse_result != 0)
        {
            ROS_WARN_THROTTLE(5.0, "UDP realtime state parse failed: %d", parse_result);
        }
    }
    else
    {
...
int main(int argc, char **argv)
{
    ...
    if(is_4th_Gen_ == 0)
    {
        ...
        sub_visual_grasp = nh_.subscribe("/visual_grasp_cmd", 10, Visual_Grasp_Callback);
    ...
    else if(is_4th_Gen_ == 1)
    {
        ...
        // Fourth-generation controllers also accept the legacy tool Modbus
        // command used by the SDK's rm_set_modbus_mode API.
        sub_setModbusMode = nh_.subscribe("/rm_driver/Set_Modbus_Mode", 10, Set_Modbus_Mode_Callback);
        pub_setModbusMode_result = nh_.advertise<std_msgs::Bool>("/rm_driver/Set_Modbus_Mode_Result",10);
        ...
        pub_readHoldingRegisters_result = nh_.advertise<rm_msgs::Register_Data>("/rm_driver/Read_Multiple_Holding_Registers_Result", 10);
        pub_readMultipleHoldingRegisters_result = nh_.advertise<rm_msgs::Register_Data>("/rm_driver/Read_Multiple_Holding_Registers_Result", 10);
        //Modbus协议写保持寄存器
    ...
    while (ros::ok())
    {
        while (realtime_arm_joint_state == true)
        {
            ...
            if ((temp[0] == 0x0A) && (buffer_cnt > 1))
            {
                ...
                if (msg == 0x0D)
                {
                    if (strstr(socket_buffer, "write_modbus_rtu_registers") != NULL ||
                        strstr(socket_buffer, "write_registers") != NULL ||
                        strstr(socket_buffer, "read_modbus_rtu_holding_registers") != NULL ||
                        strstr(socket_buffer, "read_multiple_holding_registers") != NULL)
                    {
                        ROS_INFO("tool Modbus register response: %s", socket_buffer);
                    }
                    ...
                    case MODBUS_WRITE:
                        state.data = RM_Joint.state;
                        ROS_INFO("tool Modbus write parsed: case=%d state=%s",
                                 Modbus_Write_Case, state.data ? "true" : "false");
