...
int Write_Registers_Cmd(uint8_t port, uint32_t address, uint32_t num, std::vector<uint16_t> data_content, uint32_t device)
{
    ...
    cJSON_AddNumberToObject(root, "num", num);
    for(int i =0;i<data_content.size();i++)
        cJSON_AddItemToArray(array, cJSON_CreateNumber(data_content[i]));
    cJSON_AddItemToObject(root, "data", array);
    cJSON_AddNumberToObject(root, "device", device);
    ...
    {
        ROS_ERROR("fourth-generation register request is too large: %d bytes", written);
        cJSON_Delete(root);
        free(data);
        return 1;
    }
    ROS_INFO("fourth-generation register request: %s", data);
    res = package_send(Arm_Socket, buffer, strlen(buffer), 0);
    cJSON_Delete(root);
    free(data);
    if (res < 0)
...
int Write_Modbus_Rtu_Registers(int address,std::vector<int> rtu_data,int type,int device){
    ...
    if (written < 0 || written >= static_cast<int>(sizeof(buffer)))
    {
        ROS_ERROR("Modbus register request is too large: %d bytes", written);
        cJSON_Delete(root);
        free(data);
        return 1;
    }
    ROS_INFO("tool Modbus register request: %s", data);
    res = package_send(Arm_Socket, buffer, strlen(buffer), 0);
    cJSON_Delete(root);
    free(data);
    if (res < 0)
